...
Descartes' Dreaming and Evil Demon scenarios are examples of \textit{skeptical hypotheses}. These are bizarre scenarios in which everything would seem to us as it does now, but in fact many of our beliefs would be false.
...
\begin{itemize}
	...
	\end{itemize}
	\item Descartes' Evil Demon scenario is notable for being perhaps the most extreme skeptical hypothesis, since it makes you doubt not only all your perceptual beliefs, but also even seemingly the most certain domains of inquiry like mathematics.
...
The force of skeptical scenarios depends on a crucial assumption: that a belief needs to be certain to count as knowledge. This is sometimes called the \textbf{Certainty Principle}. Without the Certainty Principle, the possibility of a skeptical scenario does not entail that we lack knowledge.

\begin{itemize}
	\item Today, most philosophers think that it's probably impossible to refute the skeptic if you grant all of her assumptions. Rather, skepticism can only be resisted by rejecting one of the skeptic's starting assumptions.
	\item Rejecting the Certainty Principle is one good way to do that.
	\item But this is not without cost. After all, the Certainty Principle is appealing. If my evidence is compatible with a scenario in which $p$ is false, do I \textit{really} know that $p$ is true??
\end{itemize}
